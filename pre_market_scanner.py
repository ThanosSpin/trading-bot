...
def submit_order_smart(
        ...
        try:
            ...
            if fresh_price and fresh_price > 0:
                ...
                if abs(price_diff) > 0.01:
                    print(
                        f"  🔄 Price update: ${current_price:.2f} → ${fresh_price:.2f} ({price_diff_pct:+.2f}%)"
                    )
                ...
                if abs(price_diff_pct) > 2.0:
                    print(
                        f"  ⚠️ WARNING: Price moved {abs(price_diff_pct):.2f}% since signal!"
                    )
            else:
    ...
    if side == "buy":
        ...
        print(
            ...
            if available_cash
            else "     Available cash: Not provided"
        )
...
def get_open_orders(symbol: str = None):
    ...
    try:
        ...
        orders = api.list_orders(status="open", symbols=symbol if symbol else None)
        return orders
...
def cancel_existing_premarket_orders():
    ...
    try:
        ...
        for order in open_orders:
            ...
            if order_time and (
                order_time.hour < 9 or (order_time.hour == 9 and order_time.minute < 30)
            ):
                api.cancel_order(order.id)
                limit_price = getattr(order, "limit_price", "N/A")
                side = getattr(order, "side", "N/A")
                print(
                    f"  🗑️ Cancelled old pre-market order: {side.upper()} {order.symbol} {order.qty} @ ${limit_price}"
                )
                cancelled_count += 1
...
def scan_and_queue_orders():
    ...
    if not is_trading_day():
        print(
            f"[INFO] {datetime.now().strftime('%Y-%m-%d')} is not a trading day. Exiting."
        )
        return []

    print(f"\n{'='*80}")
    print(
        f"🌅 PRE-MARKET SCANNER - {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')} UTC"
    )
    print(f"{'='*80}\n")
    ...
    if allow_short and not is_margin:
        ...
        if account_equity > 0:
            print(
                f"   Account equity: ${account_equity:,.2f} (need $2,000+ for margin)"
            )
    ...
    symbols = (
        PRE_MARKET_SYMBOLS
        if isinstance(PRE_MARKET_SYMBOLS, list)
        else [PRE_MARKET_SYMBOLS]
    )
        ...
        if open_orders:
            ...
            for order in open_orders:
                if order.side == "buy":
                    ...
                    print(
                        f"   • {order.symbol}: BUY {order.qty} @ ${limit_price:.2f} = ${reserved:.2f} reserved"
                    )
    ...
    for sym in symbols:
        ...
        try:
            ...
            effective_intraday_weight = (
                0.0 if session in ("pre_market", "closed") else INTRADAY_WEIGHT
            )
            ...
            print(
                f"  📈 Daily Prob: {daily_prob:.1%}"
                if daily_prob
                else "  📈 Daily Prob: N/A"
            )
            print(
                f"  ⚡ Intraday Prob: {intraday_prob:.1%}"
                if intraday_prob
                else "  ⚡ Intraday Prob: N/A"
            )
            ...
            if prob >= PRE_MARKET_MIN_PROB:
                ...
                if max_qty <= 0:
                    ...
                    print(
                        f"     Need: ${price * (1 + PRE_MARKET_LIMIT_BUFFER):.2f} per share"
                    )
                    continue
            ...
            # ============================================================
                ...
                if daily_prob is not None and daily_prob >= 0.45:
                    print(
                        f"  🛡️ PRE-MARKET SELL BLOCKED: "
                        f"daily_prob={daily_prob:.1%} is not bearish"
                    )
                    print(f"     Intraday noise may be causing false SELL signal")
                    print(
                        f"     Require daily_prob < 0.45 AND "
                        f"final_prob <= {min_sell_prob:.0%} to sell pre-market"
                    )
                    continue
                ...
                try:
                    ...
                    if current_qty > 0:
                        ...
                        # Legacy PDT opened-today restriction removed.
                        #
                        # Same-day exits are allowed. If we currently own a long
                        # position, the full long quantity may be closed.
                        sell_qty = current_qty

                        print(f"  📋 Preparing SELL order (close position):")
                        print(f"     Quantity: {sell_qty} shares")
                        ...
                        try:
                            ...
                            if order_type == "market":
                                print(f"     Type: MARKET (immediate execution)")
                                estimated_value = sell_qty * price
                            else:
                                print(f"     Type: LIMIT @ ${limit_price:.2f}")
                                estimated_value = sell_qty * limit_price
                            ...
                            print(f"  ✅ SELL order placed! " f"Order ID: {order.id}")
                    ...
                    # ========================================================
                    # No long position → optional SHORT
                    # ========================================================
                        ...
                        if allow_short:
                            if not is_shortable(sym):
                                print(
                                    f"  ⚠️ No position in {sym} "
                                    f"and not shortable - skipping"
                                )
                                continue
                            ...
                            try:
                                ...
                                print(f"     Total Value: " f"${estimated_value:,.2f}")
                                ...
                                print(
                                    f"  ✅ SHORT order placed! " f"Order ID: {order.id}"
                                )
                            ...
                            print(f"  ℹ️ No position in {sym} - " f"nothing to sell")
                            print(
                                f"     Short selling disabled: " f"need $2,000+ equity"
                            )

                # ============================================================
                # No broker position exists
                # ============================================================
                    ...
                    if allow_short:
                        if not is_shortable(sym):
                            print(
                                f"  ℹ️ No position in {sym} "
                                f"and not shortable - skipping"
                            )
                            continue

                        # Keep your existing no-position short logic here.
                        # This branch is separate from closing a long position.
                ...
                # Neutral signal
                print(
                    f"  ℹ️ Probability {prob:.1%} in neutral zone ({min_sell_prob:.0%} - {PRE_MARKET_MIN_PROB:.0%}) - skipping"
                )
    ...
    if queued_orders:
        ...
        sell_close_orders = [
            ...
            if o["side"] == "SELL" and o.get("action") == "CLOSE"
        ]
        sell_short_orders = [
            o
            for o in queued_orders
            if o["side"] == "SELL" and o.get("action") == "SHORT"
        ]
        ...
        for order in queued_orders:
            ...
            prob_str = (
                f"D:{daily:.1%} I:{intraday:.1%}"
                if daily and intraday
                else f"{order['probability']:.1%}"
            )
            ...
            print(
                f"     Signal: {order['side']}{action_label} (Probability: {order['probability']:.1%})"
            )
        ...
        # Send notification
        notification_msg = (
            f"Pre-Market Scanner queued {len(queued_orders)} order(s):\n\n"
        )
        if buy_orders:
